...
import MicroButton from '../componentes/MicroButton';  // Asegúrate de importar el componente MicroButton
...
const App = () => {
  ...
  const saveTranscription = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/my_app/guardar_transcripcion/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: `transcripcion_texto=${encodeURIComponent(dictatedText)}&traduccion_texto=${encodeURIComponent(translatedText)}`,
      });
  
      const result = await response.json();
      if (result.status === 'success') {
        console.log('Transcripción y traducción guardadas con éxito.');
        // Puedes realizar acciones adicionales después de guardar, si es necesario.
      } else {
        console.error('Error al guardar la transcripción y traducción.');
      }
    } catch (error) {
      console.error('Error de red:', error);
    }
    ...
    <div style={{ textAlign: 'center' }}>
      <div style={{ position: 'fixed', left: '50%', transform: 'translateX(-50%)', bottom: '130px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', marginTop: '10px' }}>
         ...
         <button style={{ ...buttonStyle, backgroundColor: selectedLanguage === 'pt' ? '#faab1a' : '#344955' }} onClick={() => translateText(dictatedText, 'pt')}>Portugues</button>
        </div>
        ...
        {/* Nuevo botón para guardar la transcripción */}
        <button
          style={{ ...buttonStyle, backgroundColor: '#344955', marginTop: '10px' }}
          onClick={saveTranscription}
          disabled={!dictatedText || isRecording}
        >
